...
CH4 = rows([
 ...
 ("lem:sum_decay 的关键那一步 —— 一个 Ξ 因子在球内求和，没走 ‖Ξ‖ 捷径","Q28 ✓ 关键步","done","(deccA0) 球内截断 + (1−s)ℓ_s² ≲ g²+|1−s|"),
 ("lem:sum_decay 的四步装配（sum_res_1 / (I) / (II)）","Q34 · 可开工","ready","Kernel/SumDecay.lean"),
...
<p><strong>一处小瑕疵，记在这里。</strong> 报告末行写「premises found by scanning: 10, all classified
(8 borrowed, 1 owed, 5 structural)」——<em>10 是扫出来的条数，而括号里是三份登记名单的长度</em>，
8+1+5=14，两者本来就不该相加。读者会以为那是 10 的分解。
登记名单比扫描结果长是正常的（暂时没人用的谓词扫不出来），<strong>该改的是写法</strong>。
这正是 Q26 要治的那一类「看上去完整的报告」，只是缩小版。已开 Q47。</p>
