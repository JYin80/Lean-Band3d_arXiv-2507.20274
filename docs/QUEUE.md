...
## Q24 · `ML:Kbound` — **DONE**（CC，2026-09-20）⭐：那句「额外修改」定位并证出

**文件**：`RBM3D/Loop/KBound.lean`（新）。`./check.sh` 绿、0 warning。

**工单要的两件事都做了，而且第二件比预期走得远**：

**① 陈述层**：`KLoopBound`，逐字对应 `(eq:bcal_k)`：
`∀ n ≥ 1, ∀ τ > 0, ∃ C > 0, ∀ t ∈ [0,1), ∀ σ a（长度 n），‖K^(n)‖ ≤ C L^τ (W^{-d}B_{t,0})^{n-1}`。
`≺` 按 `DetDom` 的读法展开（对每个 `τ`，常数可依赖 `d, n, g, τ`，但对 `L, t, σ, a` 一致）。
当作假设，不是 axiom；已登记进审计的 `interfaceProps`（承重目前是 0）。

**② 那句「额外修改」在哪**：在 `(eq:ind-step-bound)` 情形 (ii) 的**第 4 条**——
有两个外部边被长边分解的反对称部分 `f₁` 打到，各贡献 `(|a−b|^{d−1}+1)^{-1}`，要对 `b` 求和。
论文那一步是

`(|x|^{d−1}+1)^{-1}(|y|^{d−1}+1)^{-1} ≲ (|x|^d+1)^{-1}(|y|^{d−2}+1)^{-1} + (|x|^{d−2}+1)^{-1}(|y|^d+1)^{-1}`

**已证**：`inv_pow_pair_le`，常数 2，除 `0 ≤ x, y` 外无前提（对任意 `d = k+2` 成立）。

**为什么必须这么不对称地拆**——这是这拍的审计发现，论文没写：
如果按直觉把其中一个 `(d−1)` 因子放大成 1，剩下的 `Σ_b (|a−b|^{d−1}+1)^{-1}` 在 `d` 维格点上
是 `Σ_r r^{d−1}·r^{−(d−1)} = Σ_r 1`，直接发散；即使拆成 `d` 次幂，`Σ_b(|a−b|^d+1)^{-1} ≈ log L`
**仍然发散（对数）**。让和收敛的正是另一个因子剩下的 `d−2` 次幂，而它 `≥ 1` 当且仅当 **`d ≥ 3`**。
`d = 2` 时那个因子退化成常数、对数回来；`d = 1` 时 `|·|^{d−1} = 1` 根本不带衰减——
**这就是 `[YY_25]` 的一维论证必须改写的原因**。

**负面测试**（CLAUDE.md 规则 13）：`not_inv_pow_pair_le_single` 机器验证了
「只留拆分的第一项」不成立（`d = 3, x = 3, y = 0` 即反例），所以 `x ≤ y` / `y ≤ x` 的分情形
不是写法问题，是必须的。

**没做的（→ Q32）**：拆完之后的格点和
`Σ_b (|a−b|^d+1)^{-1}(|c−b|^{d−2}+1)^{-1} ≲ 1`（对 `a, c, L` 一致）。
手算确认它在 `d ≥ 3` 成立且量级是 `C/(|a−c|+1)`：近 `a` 处第二因子 `≈ |a−c|^{-(d−2)}`、
第一因子的和是常数；近 `c` 处第一因子 `≈ |a−c|^{-d}`、第二因子的和 `≈ |a−c|^2`；远场 `Σ_{r≥R} r^{-2}`。
零件都在 `Defs/Shells.lean` 与 `Defs/RadialSum.lean`。

**没有发现论文掩盖实质困难**：这句「额外修改」是老实的——它确实是一处需要重写的技术步骤，
而且重写的方向（拆成 `d` 与 `d−2`）在 `d ≥ 3` 下确实走得通。
...
> 审计现在报 `RBM.Loop.TwoLoopBounded: 3`——**改之前它在三个签名里看得见、在任何汇总里数不到**。
> 这条不占工单：规则 15 是硬规则，违反的是我自己上两拍写的代码，就地修掉。
> **Q26 剩下的仍是本体**：自动发现（不靠硬编码名单）+ 分两本账。
>
> **beat 16 补充（Cowork）：规则 15 立住了，但立它的那条前提还没具名。**
...
> 但这篇论文的证明**不真的需要过程**——只需要一时刻边缘律加一条生成元恒等式。
>
> **审计结论是「走得通，而且是改动预算最小的那一种」**：关键鞅引理 `lem:DIfREP` 的**陈述本来就是矩不等式**
> （两边都是 `𝔼`，BDG 只在证明里），所以论文的陈述一个字都不用改。
>
> **Q42–Q45 与维数完全无关**，RBM1D 那边是 6034 行、247 条定理、0 公理。
> 这是本项目可移植性最高的一块——比 `Loop/Unique.lean` 还干净，因为它连 `d` 都不出现。
> **顺序不要乱**：每一步都解锁下一步。
...
**关键是在全空间成立，不是「在好事件上成立」**。两个后果：Stein 要求的全局有界天然满足，
**不需要磨光截断**；并且它给出「`≺` ⟹ 矩」的反向桥（正向那座是 Markov 不等式）。
有了双向桥，`≺` 与矩可以自由来回——这是 Q45 的前提。

## Q43 · Stein 三层 ⭐ —— **OPEN**（整包可搬，与 `d` 无关）

**文件**：`RBM3D/Gauss/Stein.lean`、`RBM3D/Gauss/SteinMatrix.lean`。

三层：一维实值（`p' = −(x/v)·p`，整条路线唯一的概率内容，约 160 行）→ 一维复值（实虚部拆，约 80 行）
→ 矩阵版（约 190 行）。

**矩阵版别走 Fubini，走「重采样」**（这是 RBM1D 踩过的坑）：

```
upd c (ω, t) := 把 ω 的第 c 个坐标换成 t
((P) ⊗ γ_c).map (upd c) = P        -- 用同分布独立样本重采样一个坐标，不改测度
```

在可测长方体上验证即可（`Measure.eq_infinitePi`）。**独立性只在这一处用到。**
两个额外好处：不需要「只读有限个坐标」的假设；方差为 0 的退化情形自动覆盖
（`gaussianReal 0 0 = dirac 0`），不必加非退化假设。

## Q44 · 生成元恒等式 —— **OPEN**（需 Q43）

**文件**：`RBM3D/Gauss/Generator.lean`。

```
∂_u 𝔼[Φ(H_u)] = ½ Σ_ij S_ij 𝔼[∂_ij ∂_ji Φ(H_u)]
```

证明 = 积分号下求导（链式法则给出 `(2√u)⁻¹ Σ_α ω_α ∂_α Φ`）+ 每坐标一次 Stein
（`ω_α` 换成 `方差_α · ∂_α`，产生的 `√u` 正好消掉 `(2√u)⁻¹`）。

**用在 `|F|^{2p}` 上，二阶项长成 `|F|^{2p−2} Σ_α S_α |∂_α F|²`，
这逐字等于论文的二次变差张量 `(E ⊗ E)^{M,(n)}`**（`3_5_Loop_Hierarchy.tex:169`，
`\begin{definition}[Quadratic variation loop]`），也就是 `lem:DIfREP` 的右端。
**到这一步 Itô 就不在关键路径上了。**

## Q45 · 对矩的 Grönwall + 两座 `≺` 桥 + 连续归纳 —— **OPEN**（需 Q42、Q44）

**文件**：`RBM3D/Gauss/MomentGronwall.lean`、`RBM3D/Analysis/Bootstrap.lean`、`RBM3D/Gauss/Domination.lean`。

**硬性约定（RBM1D 实测的规模教训，照办）**：**对外一律保持 `≺`，矩只在证明内部出现。**
进证明前用反向桥（Q42）升成矩，跑完 Grönwall 用 Markov 不等式降回 `≺`。
RBM1D 实测：§5 有 141 处 `≺`，但真正需要矩形式的只有 **2 个鞅接口 + 4 处二次变差输入**。
**不要把全文 `≺` 改写成矩。**

连续归纳：`φ` 连续 + 自改进 `φ ≤ C → φ ≤ B` ⟹ `φ ≤ B`。

## Q46 · 把三处停时换成连续归纳 —— **BLOCKED**：等作者回答一个问题

**文件**：待定。**动手前先看 `docs/stochastic-audit.md` §五、§六。**

三处停时（`eq:def2_stopping`、`eq:def_TTT`、`eq:def2_stopping_BA`）**都是首次越界时刻**，
收尾都是「`T ≥ t` whp ⇒ 估计在整个确定性区间成立」——连续归纳给同样的结论，写一次用三次。

**但有一处需要作者确认**（审计 §六）：`3_5:2373` 明确用了**停止过程** `u ↦ (L−K)^{(2)}_{u∧T}`
与**停时版的积分方程** `(int_K-L_ST)`，而不只是用停时表示「第一次越界」。

> **问题**：这一处能不能改成「在确定性区间上、在归纳假设之下跑同一估计」（即改用论文自己也给了的
> 确定性 Duhamel 形式 `(int_K-LcalE)`）？
...
## Q32 · `ML:Kbound` 的格点和 — **OPEN**（CC 于 Q24 开出）

**文件**：`RBM3D/Loop/KBound.lean`（接在 `inv_pow_pair_le` 之后）。

**要证**（`d = k + 2`，`k ≥ 1`）：存在只依赖 `d` 的 `C`，对一切 `L`、`a, c ∈ Z_L^d`，

`Σ_{b ∈ Z_L^d} (|a−b|^d + 1)^{-1} (|c−b|^{d−2} + 1)^{-1} ≤ C`。

这是 Q24 拆分之后真正让和收敛的那一步，**`d ≥ 3` 在这里是充要的**
（`d = 2` 时第二个因子退化成常数，和是 `log L`）。

**路线**：按 `R := |a−c|` 分三块——`|b−a| ≤ R/2`（第二因子 `≍ R^{-(d−2)}`，第一因子的和是常数）、
`|b−c| ≤ R/2`（第一因子 `≍ R^{-d}`，第二因子的和 `≍ R²`，用 `sum_radial` + `card_sphere_le`）、
其余（两个因子都 `≳ R`，`Σ_{r ≥ R} r^{d−1}·r^{−d}·r^{−(d−2)} = Σ_{r≥R} r^{−(d−1)}`，
`d ≥ 3` 时收敛）。三块都 `≲ (R+1)^{-1} ≤ 1`。
