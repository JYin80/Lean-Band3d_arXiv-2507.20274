...
## 2026-09-20 · Claude Code · Q47 完成：审计末行拆成两行

旧写法把「扫描找到 10 条」与「三份名单共 8+1+5 条」并排放在一句里，读起来像分解、却对不上。
现在分两行：第一行是扫描结果的分类（4+1+6 = 11，自己加得起来），
第二行是账面登记数，并**点名**列出目前无人承重的 4 条
（`ThetaDiffOne`、`ThetaDiffTwo`、`KTreeRep`、`KLoopBound`）——那是「假设尚未开始承重」的信号。

**记一次操作失误**：改动时我用脚本从目标行截断到文件尾重写，把后面 Q26 的反向测试命令
`#assert_rbm_audit_detects` 一起删了，构建立刻报 `unexpected token '#'`，已补回。
**教训**：脚本里「截到文件尾」的改法，动手前先看尾巴有什么。

## 2026-09-20 · Claude Code · Q34 部分完成，并修掉 Q28 的一处缺陷

**缺陷**（值得记）：照论文证明往下读时发现，Q28 证的 `sum_ball_norm_XiKer_le`
**把球心与 `Ξ` 的第一个参数绑成了同一点**；而 `(sum_res_1_red0)` 里球心是被 δ 钉住的 `a₁`，
因子却是 `Ξ^(i)(a_i,b_i)`（`i ∉ A`）。**当初证出来的形状，用不到它要用的地方。**

**修法不需要新数学**：Q20 的 `sum_ball_min_pow_le`（截断版）本来就允许两心不同，
Q28 误用了它的同心推论 `sum_ball_pow_le`。现在球心成为独立参数，证明改用截断比较。

**新证**：`sum_prod_le_pow`（和的乘积 ↔ 乘积的和，`Finset.prod_univ_sum`，与 `Ξ` 无关）、
**`sum_prod_norm_XiKer_le`**（`(sum_res_1_red0)` 的主项：球内的 `Ξ` 因子总共值
`(CΛ²(ĝ²+|1−s|)/(ĝ²+|1−t|))^n`）。

**剩下**：`(deccA0)` 的分裂与 `W^{-D}` 误差项、`(I)`、`(II)`。

审计：**465 定理 / 171 定义 / 0 公理**。

## 2026-09-20 · Claude Code · Q35 部分完成：`n = 4` 匹配的树那一侧

`hasDerivAt_thetaEdge_sub_one`（内部边与边界边同一条求导公式，`I` 不含 `t`）、
`hasDerivAt_starFour`、以及 `treeVal_four_diag02_as_star` / `diag13_as_star`——后者说明
**把被替换的标签看作变量时，三棵树形状相同**（`Σ_b Θ(x,b)·F(b)`），对角线那两棵只需交换一次求和。

**更正我自己写的工单**：Q35 要求「四顶点版的 `sum_SB_starLeft/Right`」——**不需要**。
那两条是对任意 `F` 陈述的，`n=4` 与 `n=3` 的唯一区别是 `F` 由两条边变三条边。

**剩下**：配对本身与 `eq_on_level` 在 `n = 4`（低层已全部就位）。

审计：**469 定理 / 171 定义 / 0 公理**。

## 2026-09-20 · Claude Code · Q32 部分完成：临界指数的球和是对数

`sum_ball_inv_pow_dim_le`：`Σ_{|a−b| ≤ ρ}(|a−b|+1)^{-d} ≤ C_d(1+log(ρ+1))`。
**并排看才有意思**：同一个球，指数 `d−2` 时和是 `≍ρ²`（Q28 的 `sum_ball_pow_le`），
指数 `d` 时只剩对数——而这个对数是真的，正是让整格和长成 `log L` 的那个 `Σ1/r`，
也正是 Q24「必须不对称拆分」的原因。两条引理放在一起，这件事一眼可见。

**维护**：`sum_inv_Icc_le` 与 `inv_succ_le_log_sub_log` 从 `Kernel/SumDecay.lean`
**下沉**到 `Defs/RadialSum.lean`（依赖方向是 SumDecay → RadialSum，新引理在下层需要它们），
陈述一字未改。
...
## 2026-09-20 · Claude Code · Q41 完成：非空洞证书 —— 两条拿到，四条的障碍变成了定理

**审计数不出「假设是不是假的」**：一条假的前提会让所有带着它的定理空洞真，而审计照报
0 公理、照报依赖数。beat 6 就发生过（旧性质 5' 漏了 `σ₁ = σ₂`）。这一拍补上正面那一半。

**拿到证书**：`thetaDecay_fixedL`（`(prop:ThfadC)` 的固定 `L` 版——能证是因为它的右端含
`B_{t,K}` 的零模项 `(L^d|1−t|)⁻¹`，发散速度与粗界 `(1−t)⁻¹` **相同**）；
`twoLoopBounded_kTwoLoop`（`TwoLoopBounded` 不靠弱化，显式二圈直接满足）。

**卡在哪，写清楚**：`ThfadC_short`、`BD1`、`BD2`、`ThfadC0` **连固定 `L` 版都证不出来**。
它们的右端在固定 `L` 时对 `t` 有界，而 `Θ_t` 的元素没有界
（`not_exists_uniform_entry_bound`，机器可核）。所以要的不是更大的常数而是**相消**，
固定 `L` 下的相消 = **`1` 是 `S^(B)` 的单重特征值**（有限 `L` 谱隙）——本项目没有，**开 Q51**。
顺带检查了当年杀掉旧性质 5' 的那条行和路线对这四条**跑不动**：
`sum_Theta0_row`（`Σ_b Θ̊ = 0`）与 `sum_Theta_diff_row`（一阶差分行和 `= 0`）。

**审计新增证书列**，并修了一个加这列时撞出来的真问题：`twoLoopBounded_kTwoLoop` 的结论头
就是 `TwoLoopBounded`，`scanPremises` 把它当作「已证」**把这条前提从报告里删了**（owed 1 → 0）。
证书证的是某一个 `K`，不是定理量化的任意 `K`。现在证书不计入「已证」——
**给假设发证书不能让审计对它闭嘴。**
...
## 2026-09-20 · Claude Code · Q54：`(prop:BD1)` / `(prop:BD2)` / `PropTH` 拿到证书，审计 `6 of 9`

**一条引理管两条性质**：`exists_norm_Theta_sub_le`（`‖Θ_ξ(a,b) − Θ_ξ(a,c)‖ ≤ 2R(1−q)⁻¹`）。
差分把常模式消掉，与去零模是同一件事，Q52 的混合估计原样可用，系数 `2` 就是经 `L^{-d}` 的三角不等式；
二阶差分是两条一阶差分之和，不必重证。共用的「级数被可和项控制」抽成 `norm_tsum_le_of_le`。

**`r = 0` 那一支必须单独处理**：右端因带因子 `|r|` 而等于 `0`，任何估计都给不出来，
只能靠代数（左端也恰是 `0`）。工单提醒到了，实证如此。

顺带补了 `propTH_fixedL`（四条分量的合取）——不带新信息，但审计是逐条前提问的，
与其每次解释那一行为何空着，不如说清楚一次。`5 of 9` → **`6 of 9`**。

**剩下三条**：`ThetaDecayShort`（→ Q55；它的右端根本不含 `t`，在谱参数 `1` 处已被 `not_decayShort_at_one`
证伪，要的是 `m² ≠ 1` 的定量版，**且 `m = i`（`E = 0`）那个边界要靠 Q51 的懒惰性单独处理**）、
`KTreeRep`（真证书 = Q30/Q31 的存在性）、`KLoopBound`（等分子层）。

审计：**509 定理 / 173 定义 / 0 公理**，证书 **6 of 9**。

> **心跳已于本拍结束时按 Jun 的要求停掉**（取消了 9 分钟一次的常驻循环）。
> 下一次开工：照 `CLAUDE.md` 从 `docs/QUEUE.md` 认领第一条 OPEN（当前是 **Q55**）。
