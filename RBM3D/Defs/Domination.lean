...
Definition `stoch_domination` (ii) of *Delocalization of non-mean-field random
matrices in dimensions `d ≥ 3`* (arXiv:2507.20274).

For two deterministic `N`-dependent quantities the paper writes `ξ ≺ ζ` if
`ξ ≤ N^τ ζ` for every `τ > 0` and all sufficiently large `N`.

This file is taken verbatim from the sister project `RBM1D`, whose Definition 2.1 (ii)
is the same definition; only this header differs.

Part (i) of the definition is uniform in a possibly `N`-dependent parameter
`u ∈ U(N)`, and the deterministic estimates of the paper -- `(prop:BD1)`,
`(prop:BD2)`, `(prop:ThfadC0)`, which hold uniformly in the lattice points -- are
used in that uniform form.  We
