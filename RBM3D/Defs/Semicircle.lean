...
`(eq:defmzsc)`: `m(z) = m_sc(z) = (-z + √(z²-4))/2`, the Stieltjes transform of the
semicircle law, and `(eq:zt)`: the spectral parameter flow `z_t = E + (1-t) m(E, ĝ)`.

* `m^{(E)}` is the solution of `m(m + E) = -1` with `Im m > 0`; for `|E| < 2` it is
  `(-E + i√(4 - E²))/2` and **has modulus `1`** -- which is exactly the hypothesis
  `‖m‖ = 1`, `0 < m.im` that the interface `Prop`s of `Propagator/Interface.lean` carry, so
  this file is where a concrete witness for them comes from.
* `m_sc(z)` is the root of `m² + z m + 1 = 0` with `Im m > 0`, for `Im z > 0`.
* `z_t^{(E)} = E + (1 - t) m^{(E)}`, and `η_t = Im z_t = (1-t) Im m^{(E)} > 0` for `t < 1`
  and `|E| < 2`, which is `(eta)`.
* The change of variables of the sister paper's Lemma 2.8: for `Im z > 0` put
  `E = -2 Re m_sc(z)/|m_sc(z)|` and `t = |m_sc(z)|²`; then `m^{(E)} = m_sc(z)/|m_sc(z)|`,
  `0 < t < 1`, `|E| < 2`, `m_sc(z) = t^{1/2} m^{(E)}` and `z = t^{-1/2} z_t^{(E)}`, with the
  quantitative form `|E| ≤ |Re z|`, `t ≥ (1+|z|)⁻²`, `Im z_t = t^{1/2} Im z`.

This is `RBM1D/Defs/Semicircle.lean` with its docstrings re-pointed at this paper: `m_sc`
is scalar and dimension-independent, so nothing in it changes with `d`.
-/
namespace RBM
