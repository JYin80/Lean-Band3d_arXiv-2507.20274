...
/-!
# The moment route: moments imply `≺`

`(stoch_domination)` is a statement about probabilities of failure events, while every
estimate on the no-Itô route is proved for moments.  This file is the **forward** bridge
between them, by Markov's inequality: a family whose `2p`-th moments are bounded by
`C(ε,p) N^{εp} Φ^{2p}` for every `ε > 0` and `p` is stochastically dominated by `Φ`,
provided the parameter set is polynomially large (the union bound of
`RBM.StochDom.of_forall_le`).

The **reverse** bridge -- `≺` together with a deterministic envelope implies moments -- is
`docs/QUEUE.md`, Q42b; its analytic input is `RBM3D/Analysis/Resolvent.lean`.

Also here: the net argument that upgrades a bound at polynomially many points of an
interval to a bound on the whole interval, for Hölder or Lipschitz families.  It is what
lets a statement proved at each fixed time be quoted uniformly in `t`.

This file is `RBM1D/Gauss/Domination.lean` with its docstrings re-pointed at this paper;
the mathematics is dimension-independent.
-/
namespace RBM.Gauss
