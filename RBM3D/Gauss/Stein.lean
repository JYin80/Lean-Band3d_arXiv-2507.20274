...
The analytic foundation of the **no-Itô route** for the stochastic layer of this project
(`docs/stochastic-audit.md`, and `CLAUDE.md` rule 10).  Realizing the flow of `(eq:zt)` as
`H_u = √u · X` with `X` a fixed Gaussian band matrix, the generator identity

  `d/du E[Φ(H_u)] = (1/2) Σ_ij S_ij E[∂_ij ∂_ji Φ(H_u)]`

is what replaces Itô's formula, and *this* file is what makes that identity true: the only
probabilistic input anywhere on the route is that a centred Gaussian density satisfies
`p' = -(x/v)p`, so that one integration by parts on the line turns a factor of `x` into a
derivative.

Mathlib has `gaussianPDFReal` with an explicit formula but no integration-by-parts lemma,
so it is proved here.

`d` does not appear in this file at all -- it is one-dimensional analysis about the real
line.  It is taken over verbatim from the sister project `RBM1D` (`RBM1D/Gauss/Stein.lean`),
which imports nothing of its own either; only this header differs.

## Main results

* `RBM.hasDerivAt_gaussianPDFReal_zero` : `p' = -(x/v)p`
* `RBM.integral_mul_gaussianPDF` : Stein's identity, density form
* `RBM.integral_mul_gaussianReal` : Stein's identity, `E[Xf(X)] = v E[f'(X)]`
* `RBM.integral_mul_gaussianReal_of_bdd` : the form with the hypotheses the matrix version
  actually supplies (`f` bounded and `C¹`)
* `RBM.integral_mul_gaussianReal_complex` : the `ℂ`-valued form

## Note on hypotheses

Integrability is an explicit hypothesis rather than a consequence of growth conditions.
In every application the integrand is a polynomial in entries of `G = (H - z)^{-1}` with
`Im z ≥ η > 0`, so `‖G‖ ≤ η⁻¹` holds on the *whole* space and every derivative is bounded
globally; integrability against a Gaussian is then immediate.  Keeping it as a hypothesis
makes this file reusable and keeps the domination argument where it belongs.
-/
namespace RBM
